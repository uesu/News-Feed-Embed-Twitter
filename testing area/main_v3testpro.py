...
#
# Same behavior as main_v2.py, but the action row is NESTED INSIDE the
...
# (round-9 behaviour, unchanged). Set it to the largest video size the
# "testing area/video_diag.py" test proved Discord can play, e.g.
#   GALLERY_VIDEO_LIMIT = 100 * 1024 * 1024   # 100 MiB
# When active, videos above it are downgraded to the largest variant at or
# below it (with the usual "showing a smaller version" note), or get a
# "watch on X" note if no variant fits — so the gallery never shows a dead
# "image not found" tile.
...
async def collect_media(session: aiohttp.ClientSession, tweet_like: dict) -> tuple[list, list]:
    ...
    Order of decisions per video:
      • GIF (type=gif / tweet_video)  -> animated image if a converter answers
                                         (gif.fxtwitter .webp, else fastgif
                                         .gif), otherwise mp4 handling
