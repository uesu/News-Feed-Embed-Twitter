# ---------------------------------------------------------------------------
# 📋 Full Components V2 Edition — main_v2_buttoninside.py
# ---------------------------------------------------------------------------
# This is a separate file — your V1 main.py stays untouched
# To use it: change the workflow's run: python main.py
# line to run: main_v2_buttoninside.py
# Note: buttons are inside the component v2
# ---------------------------------------------------------------------------
import os
import re
import json
...
# Style 5 = Link button. Emoji can be Unicode {"name": "🔗"} OR custom
# {"id": "123...", "name": "emojiname", "animated": False}
# ---------------------------------------------------------------------------
...
STATIC_BUTTONS = [
    {
        ...
        # 👇 Example custom emoji format — replace id/name with YOUR OWN emoji
        # to properly test custom emoji rendering. See README for how to get
        # your emoji's ID.
...
def build_action_row(read_post_url: str) -> dict:
    """Builds the button row. Used both standalone (V1) and nested (V2)."""
...
def build_v2_container(account: str, tweet: dict, read_post_url: str) -> dict:
    ...
    # 🔘 Divider + Buttons nested INSIDE the container (not a separate top-level row)
    container_components.append({"type": 14, "divider": True, "spacing": 1})
...
async def send_v2_webhook(session: aiohttp.ClientSession, webhook_url: str,
    ...
    # Buttons now live INSIDE the container, so payload only needs the container itself
    payload = {
